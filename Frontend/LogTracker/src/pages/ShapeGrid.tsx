...
const ShapeGrid: React.FC<ShapeGridProps> = ({
  ...
  const hoverPositionRef = useRef<GridOffset | null>(null);
  ...
  useEffect(() => {
    ...
    const drawGrid = () => {
      ...
      gradient.addColorStop(1, 'rgba(9, 7, 12, 0.65)');

      ctx.fillStyle = gradient;
      ctx.fillRect(0, 0, canvas.width, canvas.height);

      if (hoverPositionRef.current && isHex) {
        const { x, y } = hoverPositionRef.current;
        ctx.save();
        ctx.shadowBlur = 28;
        ctx.shadowColor = 'rgba(255, 107, 53, 0.95)';
        ctx.globalAlpha = 0.95;
        drawHex(x, y, squareSize * 1.18);
    ...
    const handleMouseMove = (event: MouseEvent) => {
      ...
      hoverPositionRef.current = { x: mouseX, y: mouseY };
